...
Everything below was either read out of the vendor's own SDK source, read out
of the decrypted SC3 firmware, or measured on the device. Where those disagree
it is called out.
...
* **`0xFD`** falls into the dispatcher's default arm with `idx = 124`, i.e. it
  ...
  source behind it and should not be repeated.)
...
| `0x0C` | I2S1 | answers |
| `0x0D` | SPDIF | answers |
| `0x0E` | GPIO | **does not answer.** See below |
...
`0x11` appears in the vendor flash tool as a flash/boot-mode control. It does
not answer in normal operation on the SC3. Boot mode is entered by a completely
different mechanism (a 9-byte `HidD_SetFeature` handshake beginning `00 AA`),
and the bootloader that comes up does not speak ACP at all.
...
**`ACP address = 0x81 + index`** into the effect table. Confirmed three ways:
by the vendor SDK (`DEMO_gain_control0_ADDR = 0x81`; `count = DEMO_COUNT_ADDR -
0x81`), by the decrypted firmware (`addi $r16, $r0, #-129` at the node
accessor), and structurally on the device: `0x9D`-`0x9F` are 3 nodes of 18
parameters (the three DRCs), `0xA0`-`0xA6` are 7 nodes of **52** parameters,
and the SDK documents `eq` as having exactly 52.
...
The single-parameter form needs **`LEN = 3`**, not 2. A read-modify-write
through the `0xFF` bulk form round-trips byte for byte.

ACP writes land in live registers and a power cycle reverts them.

### Gain nodes

The sixteen nodes `0xA7`-`0xB6` are `gain_control`, effect type `0x0F`, proven
by the registration loop in the decrypted firmware (16 iterations, `movi55
$r4,#0xf`, ACP addresses `0xA7..0xB6`).

They report `status` plus two parameters; the gain is the second, at **raw
report offset 10**. Values are **linear Q12 with 4096 = unity**, applied as
...
clamped to -9000..+1200; measured values of 4096 / 3254 / 7284 / 0 are
impossible in that range. Treat the SC3 as linear Q12.

Observed resting values: `0xA7`-`0xAF` 4096, `0xB0` (Usb Out) 7284,
`0xB3`-`0xB5` 3254, `0xB6` whatever the line-in fader is set to.
...
### `0x0E` does not answer, and "zeros" was the stale cache

Earlier notes here recorded `0x0E` as answering with all zeros, and explained it
by an empty handler body. Both halves were wrong, and the way they were wrong is
the trap this document warns about at the top.

There is no `0x0E` arm in the dispatcher. `0x0E` falls into the default, where
`idx = 0x0E + 0x7F = 0x8D` fails the `idx < 0x7D` check and jumps to `0x259B2`.
The only reply builder reachable from there needs `LEN != 0`, so a `LEN = 0`
read of `0x0E` cannot construct a reply at all.

Measured on the device, with the control-byte match enforced and a known-good
control read either side of it:

```
0x01 system (known good)   -> ff 00 00 00 00 01 00 07 00 01 00 00 00 01 00
0x0E GPIO                  -> NO REPLY
0x01 again                 -> ff 00 00 00 00 01 00 07 00 01 00 00 00 01 00
0x0E GPIO                  -> NO REPLY
0x0E GPIO                  -> NO REPLY
```

The zeros seen originally were `GetInputReport` re-serving a previous reply. Any
client that does not compare the reply's control byte to the request will read
them as an answer.
...
0x0257D0  Communication_Effect_Config(r0 = CTRL, r1 = body, r2 = len)
          switch on 0x00 0x01 0x02 0x03 0x04 0x06 0x07 0x08 0x09 0x0A 0x0B 0x0C 0x0D
                    0x80 0xFC 0xFF        (no arm for 0x05 or 0x0E)
...
54-entry table at flash `0x123012` does satisfy the structural filter and
implies a plausible `$gp`, but its values are a monotone staircase and the
implied context table holds packed u16 pairs rather than pointers. It is a
coincidence and was rejected.)
...
```sh
# from your own decrypted image, no hardware needed
python -m decrypt decrypt FIRMWARE.MVA -o plain.bin
python tools/effect_table.py plain.bin

# or ask the device: control 0x80 with a 1-based index returns that node's name
python tools/sc3_nodes.py names

# both at once, to cross-check them (this one does need the device)
python tools/sc3_nodes.py names --image plain.bin
```
...
* **No memory read primitive.** The firmware-side dispatcher handles only a
  fixed set of controls and **no command carries an address operand**. Probing
  every responsive control with 1- and 4-byte payloads found only `0x80` to be
...
* **No audio-path routing control.** Nothing in the config space selects or
  switches a path. `PhubPathSel` is compiled into the code image and set at
  init. The persisted flash configuration covers `0x00`, `0x01`, `0x03`-`0x0D`,
  `0x80`, `0x81`-`0xFB` and `0xFC`, with no path entry.
